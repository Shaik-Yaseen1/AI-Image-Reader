...
function initializeEventListeners() {
    ...
    // Only trigger file dialog when clicking the empty area (not the button)
    uploadArea.addEventListener('click', (e) => {
        // If click originated from inside the upload-area but not on the choose button
        if (!e.target.closest('#chooseBtn')) {
            fileInput.click();
        }
    });
    // Explicit button click that won't bubble back to uploadArea
    if (chooseBtn) {
...
function processFiles(files) {
    ...
    if (imageFiles.length === 0) {
        ...
        // Reset input so user can reselect immediately (including same files)
        fileInput.value = '';
        return;
    }
    ...
    showControls();
    renderImages();
    // Reset input so selecting the same files again will fire change event
    fileInput.value = '';
}
...
// Go back to previous view or upload section
function goBack() {
    if (currentView === 'rag') {
        switchView('grid');
